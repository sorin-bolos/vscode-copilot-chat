...
class ReadFileTool implements ICopilotTool<ReadFileParams> {
	...
	async prepareInvocation(
		...
		try {
			...
			void this.sendReadFileTelemetry('invalidFile', options, {
				start: 0,
				end: 0,
				truncated: false,
			});
	...
	public alternativeDefinition():
		...
		if (
			this.configurationService.getExperimentBasedConfig<boolean>(
				ConfigKey.Internal.EnableReadFileV2,
...
class ReadFileResult extends PromptElement<ReadFileResultProps> {
	...
	override async render() {
		await this.instantiationService.invokeFunction((accessor) =>
			assertFileOkForTool(accessor, this.props.uri),
		);
		...
		return (
			<>
				...
				!this.props.truncated ? undefined : (
					<>
						File: `
						{this.promptPathRepresentationService.getFilePath(
							this.props.uri,
