			...
			children.push(
				...
				>
					<ToolResultElement
						toolCall={toolCall}
						toolInvocationToken={
							this.props.promptContext.tools!.toolInvocationToken
						}
						toolCallResult={
						...
						validateInput={
							round.toolInputRetry < MAX_INPUT_VALIDATION_RETRIES
						}
...
class ToolResultElement extends PromptElement<ToolResultElementProps, void> {
	...
	async render(
		...
		if (!this.props.toolCallResult && !this.props.allowInvokingTool) {
			throw new Error(
				`Missing tool call result for "${this.props.toolCall.id}" (${this.props.toolCall.name})`,
			);
		}
		...
		const copilotTool = this.toolsService.getCopilotTool(
			this.props.toolCall.name as ToolName,
		);
		if (toolResult === undefined) {
			...
			if (this.props.validateInput) {
				const validationResult = this.toolsService.validateToolInput(
				...
				if ('error' in validationResult) {
					...
					extraMetadata.push(
						new ToolFailureEncountered(this.props.toolCall.id),
					);
			...
			if (toolResult === undefined) {
				try {
					if (
						...
						throw new Error(
							`Tool ${this.props.toolCall.name} is currently disabled by the user, and cannot be called.`,
						);
					}
					...
					toolResult = await this.toolsService.invokeTool(
						this.props.toolCall.name,
						{
							input: inputObj,
					...
					if (errResult.isCancelled) {
						...
						this.logService.error(
							`Error from tool ${this.props.toolCall.name} with args ${this.props.toolCall.arguments}`,
	...
	private async sendToolCallTelemetry(
		invokeOutcome: ToolInvocationOutcome,
		validateOutcome: ToolValidationOutcome,
	) {
		const model =
			this.props.promptContext.request?.model &&
			(
