...
import {
	IHeatmapService,
	SelectionPoint,
} from '../../../../platform/heatmap/common/heatmapService';
...
import {
	...
	summarizeDocuments,
} from './summarizedDocument/summarizeDocumentHelpers';
...
export async function summarizeTemporalContext(
...
): Promise<ReadonlyMap<string, DocumentProjectionPair>> {
	...
	const maxAgeMillis =
		...
		clamp(
			configService.getExperimentBasedConfig(
				ConfigKey.Internal.TemporalContextMaxAge,
				expService,
			),
			0,
	...
	const preferSameLang = configService.getExperimentBasedConfig(
		ConfigKey.Internal.TemporalContextPreferSameLang,
		expService,
	);
		...
		tokenBudget,
		{
			costFnOverride: (node, currentCost, snapshot) => {
				...
				const langCost =
					preferSameLang &&
					currentDocs.some(
						(doc) => snapshot.languageId !== doc.languageId,
					)
						? 1
						: 0;
				...
				for (const point of points) {
					if (node.range.contains(point.offset)) {
						const age =
							timespawn &&
							1 - (point.timestamp - minTimestamp) / timespawn;
	...
	for (let i = 0; i < documents.length; i++) {
		...
		map.set(input[i].document.uri.toString(), {
			doc: input[i].document,
			projectedDoc: documents[i],
		});
	}
...
export class TemporalContext extends PromptElement<TemporalContextProps> {
	...
	async render(_state: void, sizing: PromptSizing) {
		const enabled =
			...
			(this.props.location === ChatLocation.Editor
				? this.configurationService.getExperimentBasedConfig(
						ConfigKey.TemporalContextInlineChatEnabled,
						this.experimentationService,
					)
				: this.configurationService.getExperimentBasedConfig(
						ConfigKey.TemporalContextEditsEnabled,
						this.experimentationService,
					));
		...
		return (
			<Tag name="recentDocuments">
				I have read or edited some files recently. They may be helpful
				...
				{Array.from(
					Iterable.map(
						documents,
						([_, { doc: origDoc, projectedDoc }]) => {
							return (
								<>
									<Tag
										name={
											projectedDoc.isOriginal
												? 'document'
												: 'documentFragment'
										}
									>
										...
										<CodeBlock
											...
											languageId={origDoc.languageId}
											uri={origDoc.uri}
											code={projectedDoc.text}
										/>
									</Tag>
									<br />
								</>
				...
				<meta
					value={
						new TemporalContextStats(
							documents.size,
							Iterable.reduce(
								documents.values(),
								(p, { projectedDoc: c }) => p + c.text.length,
								0,
							),
						)
					}
