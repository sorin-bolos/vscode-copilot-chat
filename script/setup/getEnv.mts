...
async function fetchSecrets(): Promise<{ [key: string]: string | undefined }> {
	const developmentSecretClient = await setupSecretClient(
		'https://copilot-development.vault.azure.net/',
	);

	const secrets: { [key: string]: string | undefined } = {};
	secrets['HMAC_SECRET'] = await fetchSecret(
	...
	if (!process.stdin.isTTY) {
		// only in automation
		const automationVaultClient = await setupSecretClient(
			'https://copilot-automation.vault.azure.net/',
		);
		secrets['GITHUB_OAUTH_TOKEN'] = await fetchSecret(
			automationVaultClient,
			'capi-oauth',
		);
		secrets['GHCR_PAT'] = await fetchSecret(
			automationVaultClient,
			'ghcr-pat',
		);
